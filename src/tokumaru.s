; Decompressor by Bisqwit under zlib license.

.include "globals.inc"
.export decompress_tokumaru
...
Next          = 4  ; 4 bytes
BitBuffer     = 8  ; 1 byte
RowsRemaining = 9  ; 1 byte
.if RAMTEMP=1
    TilesRemaining = 10 ; 1 byte
    ReadBit_Atemp  = 11 ; 1 byte
    ReadBit_Ytemp  = 12 ; 1 byte
.endif

SourcePtr = ptr_temp

Plane0 = tokumaru_storage+4
Plane1 = tokumaru_storage+5
.if RAMBUFFER=1
Part2 = Plane1 ; 8 bytes, overlapping with Plane1
.endif
...
.error "don't use"
ColorN = tokumaru_storage+2 ; 1 byte
ColorA = tokumaru_storage+3 ; 1 byte
.else
